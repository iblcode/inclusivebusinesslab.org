...
export function mount (ctx, props, el) {
  ...
  const updateValue = () => {
    ...
    if (value != null) {
      while (el.lastChild.id !== 'story-video-select-span') { el.removeChild(el.lastChild) }
      el.insertAdjacentHTML( 'beforeend', value )
      input.value = value
    }

    if ( value == null || value == '' ) { el.classList.add("empty-video") }
    else {
      if ( value.indexOf("placehold.it") !== -1) { el.classList.add("empty-video") }
      else { el.classList.remove("empty-video") }
    }
  }
  ...
  input.addEventListener("input", function (e) {
    ...
    store.dispatch(update({[props.field] : cleaned}))
  })
...
<span id="story-video-select-span" style="z-index: 999; position: absolute; background-color: rgba(0, 0, 0, 0.65); border-radius: 0px 0px 10px 10px; left: 50%; top: 0; margin-right: -50%; transform: translate(-50%, 0);">
  <p style="padding: 15px; color: white;">Paste Link:
...
function clean(string) {
  ...
  var target = container.querySelector('iframe')
