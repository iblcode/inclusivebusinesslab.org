ibl = {
  ...
  isotope: {
    buildMenu: function(root) {
      ...
      var menu =
        '<div class="isotope-menu"><nav><span data-filter="*" class="active">All</span>'
      var arr = []
    ...
    init: function() {
      $('.isotope2').each(function(i, el) {
        $(el)
          ...
          .each(function(ii, elem) {
            if (
              elem.querySelector('[data-date]').dataset.date >
      ...
      $('.isotope,.isotope2').each(function() {
        ...
        var $grid = (this._grid = $('.isotope-content', this).isotope({
        ...
        $(window).on('resize scroll', function() {
          if (scope[0]._pause == false) {
            ...
            if (
              $(this).scrollTop() >=
              scope.offset().top - $(this).height() / 2
            ) {
              scope[0]._isocurr += 10
              $(
                '.isotope-item:lt(' + scope[0]._isocurr + ')',
                scope[0]
              ).addClass('active')
...
$(document).ready(function() {
  $(document.body).addClass(
    'is-touchscreen-' + ('ontouchstart' in window || !!navigator.maxTouchPoints)
  )
  ...
  $('.team-member:not(.notClickable .team-member)')
    .not('.js-visible')
    .click(function() {
